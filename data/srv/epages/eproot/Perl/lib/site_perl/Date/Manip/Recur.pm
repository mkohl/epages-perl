...
sub _init {
   ...
   $$self{'data'}{'noint'}    = 1;     # 0 if an interval is present
                                       # 1 if no interval is present and dates not done
                                       # 2 if no interval is present and dates done
...
sub modifiers {
   ...
   foreach my $flag (@flags) {
      next  if ($flag =~ /^([pn][dt][1-7]|[fb][dw]\d+|cw[dnp]|[npd]wd|easter)$/);
...
sub next {
   ...
   if (defined $$self{'data'}{'curr'}) {
      while (1) {
         $$self{'data'}{'curr'}++;
         if ($$self{'data'}{'noint'}) {
            return (undef,0)  if (! exists $$self{'data'}{'dates'}{$$self{'data'}{'curr'}});
         }
