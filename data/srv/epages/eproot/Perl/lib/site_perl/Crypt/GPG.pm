...
#    $message .= "\n" unless $message =~ /\n$/s;
    ...
    my $y = $self->gpgbin . " @opts --marginals-needed " . $self->marginals . " --status-fd 1 --logger-fd 1 --command-fd 0 --no-tty --verify $tmpnam3 $tmpnam";
    $x = `$y`;
  }
  ...
  -f $tmpnam && unlink $tmpnam;
  -f $tmpnam2 && unlink $tmpnam2;
  -f $tmpnam3 && unlink $tmpnam3;
...
#            or $_->{Keyring} eq "$tmpdir/pubring.gpg") ? $_ : 0 }
    ...
    system("copy /Y $tmpnam $tmpnam~ >nul") if $Config{osname} eq 'MSWin32';
    backtick($self->gpgbin, @opts, '-v', '--import', "$tmpnam~");
    unlink("$tmpnam~") if $Config{osname} eq 'MSWin32';
  }
  rmtree($tmpdir, 0, 1);
  unlink($tmpnam);
  return @ret;
